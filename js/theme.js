function initThemeSystem() {
  ...
  // 1. Load Preference
  const savedTheme = localStorage.getItem('boratime_theme_pref') || 'auto';
  if(themeSelector) {
  ...
  window.matchMedia('(prefers-color-scheme: dark)').addEventListener('change', e => {
     if(localStorage.getItem('boratime_theme_pref') === 'auto' || !localStorage.getItem('boratime_theme_pref')) {
        applyTheme('auto');
     }
  });
  ...
  if(themeSelector) {
     themeSelector.addEventListener('change', (e) => {
         ...
         localStorage.setItem('boratime_theme_pref', newTheme);
         applyTheme(newTheme);
...
(function checkInitialTheme() {
   const pref = localStorage.getItem('boratime_theme_pref');
   if(pref) { 
      let t = pref;
